...
Dieses Repository enthält ein strukturiertes Bash-Skript (`demo_pg_recover.sh`), das den kompletten Workflow von PostgreSQL Point-in-Time Recovery (PITR) simuliert. Es erstellt einen Test-Cluster, generiert Testdaten, sichert diese Daten mit WAL-Archiven, simuliert ein Desaster, und führt anschließend eine Recovery durch, um die Datenintegrität sicherzustellen.
...
1. **Cluster Initialisierung**: Ein temporärer PostgreSQL-Cluster wird in `/tmp/pgdata_test` erstellt.
2. **Datenbank & Tabelle erstellen**: Testdatenbank und Tabelle mit Beispiel-Datensätzen.
3. **WAL-Archivierung**: Konfiguration zur Sicherung von Write-Ahead-Logs für PITR.
4. **Base Backup & Verification**: Sicherung des Datenbestands und Überprüfung der Backup-Integrität.
...
Das Skript läuft eigenständig und führt folgende Schritte aus:
...
3. Führt ein WAL-basiertes Base Backup durch.
